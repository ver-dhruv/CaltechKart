...
aux_t *aux_init(double force_const, list_t *bodies) {
...
void test_force_creator_aux() {
  ...
  gravity_aux->bodies = list_init(0, NULL);
  ...
  drag_aux->bodies = list_init(0, NULL);
...
void remove_body(void *aux) {
  scene_aux_t *scene_aux = aux;
  scene_t *scene = scene_aux->scene;
...
void count_calls(void *aux) {
  ...
  assert(scene_bodies(scene_aux->scene) == 3 - count);
...
void test_reaping() {
  ...
  scene_aux_t *scene_aux = malloc(sizeof(scene_aux_t));
  scene_aux->scene = scene;
  scene_aux->force_const = 0;
  scene_aux->bodies = list_init(0, NULL);
  list_t *list = list_init(0, NULL);
  scene_add_bodies_force_creator(scene, remove_body, scene_aux, list);

  scene_aux_t *count_aux = malloc(sizeof(scene_aux_t));
  count = 0;
  count_aux->scene = scene;
  count_aux->force_const = 0;
  count_aux->bodies = list_init(2, NULL);
  list_add(count_aux->bodies, scene_get_body(scene, 0));
  list_add(count_aux->bodies, scene_get_body(scene, 1));
